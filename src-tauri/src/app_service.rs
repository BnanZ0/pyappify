...
async fn build_app_shortcut_command(
    ...
    if profile.main_script.ends_with(".py") {
        ...
        let bootstrap =
            build_app_shortcut_bootstrap(app_name, profile, &path::path_to_abs(&script_path))?;
...
fn build_app_shortcut_bootstrap(
    app_name: &str,
    profile: &Profile,
    main_script_path: &str,
) -> Result<String> {
    let env_names = serde_json::to_string(&process::PYTHON_ENVS_TO_REMOVE)?;
    let main_script = serde_json::to_string(main_script_path)?;
    let python_path = serde_json::to_string(&profile.python_path)?;
    let app_name = serde_json::to_string(app_name)?;
    let show_error_dialog = if profile.use_pythonw() {
        "True"
    } else {
        "False"
    };
...
_app_name = {app_name}
_show_error_dialog = {show_error_dialog}
...
def _show_failure(message):
    ctypes.windll.user32.MessageBoxW(
        None,
        message,
        f"{{_app_name}} - Application Error",
        0x00000010 | 0x00010000,
    )

try:
    _result = subprocess.run([sys.executable, _main_script], env=os.environ)
...
mod tests {
    ...
    use super::{
        ...
        is_invalid_json_error, parse_app_preferences, resolve_current_version_state,
    };
    ...
    fn shortcut_profile(use_pythonw: bool) -> Profile {
        Profile {
            name: "default".to_string(),
            main_script: "main.py".to_string(),
            admin: None,
            use_pythonw: Some(use_pythonw),
    ...
    fn pythonw_shortcut_supervises_native_crashes_without_a_second_error_dialog() {
        let bootstrap = build_app_shortcut_bootstrap(
            ...
            r"C:\apps\example\main.py",
        )
        .unwrap();

        assert!(bootstrap.contains("_show_error_dialog = True"));
        assert!(bootstrap.contains("_SEM_NOGPFAULTERRORBOX"));
    ...
    #[test]
    fn console_python_shortcut_keeps_exec_replacement_behavior() {
        let bootstrap = build_app_shortcut_bootstrap(
            "Example App",
            &shortcut_profile(false),
            r"C:\apps\example\main.py",
        )
        .unwrap();

        assert!(bootstrap.contains("_show_error_dialog = False"));
        assert!(bootstrap.contains("os.execve"));
    }
